...
- Name: customer-registration.spec.ts >> Customer Registration >> should show an error message when email is already taken
- Location: tests/customer-registration.spec.ts:55:7
...
Call log:
  ...
  - waiting for getByTestId('name-input')
    14 × locator resolved to <input id="name" type="text" required="" value="First User" data-testid="name-input" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"/>
       - unexpected value "notEmpty"

```

```yaml
- textbox "Name": First User
```
